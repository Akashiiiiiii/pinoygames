...
function pinoygames_plugin_assets() {
    // CSS and JS Files
    wp_enqueue_style("tregs_style", //unique name for css file
    PLUGIN_URL."/tregs/home/assets/css/popup_style.css", // css file path
    '', // dependency on other files
    PLUGIN_VERSION, //plugin version number
    );


    wp_enqueue_script("pinoygames_script_popup2", //unique name for js file
    PLUGIN_URL."/tregs/home/assets/js/popup2_script.js", // js file path
    '', // dependency on other files
    PLUGIN_VERSION, //plugin version number
    false
    );

    wp_localize_script("pinoygames_script_popup2","ajaxurl",admin_url("admin-ajax.php"));

 
}

add_action("init","pinoygames_plugin_assets");
...
add_shortcode("popup_shortcode","popup_pluginfunction");

function popup_pluginfunction(){
   // attached shotcode file
   include_once PLUGIN_DIR_PATH."/home/assets/shortcode/shortcode_rtg.php";
}


  add_shortcode("pinoy_games_shortcode","pinoy_games_pluginfunction");

 function pinoy_games_pluginfunction(){
    // attached shotcode file
